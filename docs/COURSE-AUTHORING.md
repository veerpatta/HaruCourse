## Modules 1–5 action contract — 12 September 2026
...
Desktop may place task context beside a readable answer column. Phones keep one column, wrapping labels, at least 44 px controls, 16 px editable text and a bounded lesson overview. Timer controls keep a stable footprint while starting or pausing. Verify real clicks, long answers, repair selection, saved resume and offline recovery as well as width. Keep Haru observation, native phone checks and formal mastery separate from implementation evidence. Modules 6–20 retain their previous teaching until individually reviewed and supplied with action plans.
...
- Preserve the five legacy step indices used by session logs. Lesson.flow contains stable action IDs, section IDs, legacy step attribution and references to worksheet fields or supplied checks. Lesson 1 is authored in src/lessonOneFlow.ts; other reviewed flows use src/lessonActions.ts and their explicit plans. Each of the 616 Module 1–5 field IDs appears exactly once in its lesson flow. Labels may change; IDs may not.
...
Modules 6–20 inherit shared time, Check persistence and explicit completion controls only. Review each lesson before adding its flow; do not mark a lesson redesigned from shared UI alone. Keep historical verification evidence and add current results to PROGRESS.md and the relevant verification document.
