...
For future implementation, a short instruction is insufficient if the learner cannot explain the terms, picture the result or find the workspace. Use a familiar situation, plain meaning, visible worked reasoning, supported attempt and independent variation. Keep essential examples and connected short explanations in the main path. This supersedes older rules that put connected prose or all longer explanations only in optional details. Editorial length suggestions in the plan are hypotheses to test, not reasons to cut required work or hide essential teaching. Retain the action reader, exact saved position and current field/question/action meanings.

Optional AI learning may explain a concept after course teaching before an independent assignment; critique/rehearsal uses a learner attempt. Never make a confused learner finish an assignment before receiving explanation. Each prompt must be fully authored for the lesson, contain fixed labelled practice material and a help/stop protocol, and name an existing return field. Keep the local alternative complete and AI use outside completion requirements. No AI or worked teaching for the diagnostic; independent project decisions remain learner-owned. Use the existing AI/content/record authorities rather than a second prompt store or progress source. Pilot understanding and resumption before rolling out the new pattern; do not infer learner validation from structural checks.

## All-course action contract — 13 September 2026

The creator approved continuing the saved-action reader through Module 20. All 224 published teaching lessons now expose their existing teaching and guided practice as saved Learn → Do → Check → Your work actions. `src/moduleActionPlans.ts` keeps the explicitly reviewed Module 1–5 routes and repair mappings. From Module 6 onward, `src/lessonActions.ts` uses each lesson's already-authored recommended and alternative routes, then resolves each Check to the worksheet step named by that lesson's own repair instruction. Lesson 1 retains its original action source and IDs. The app, server and generated Markdown consume the same resulting flow. See [VERIFICATION-ALL-ACTIONS.md](VERIFICATION-ALL-ACTIONS.md) for current evidence and [VERIFICATION-MODULE-ACTIONS.md](VERIFICATION-MODULE-ACTIONS.md) for the historical first-five-module release.

## Previous Modules 1–5 action contract — 12 September 2026

The creator approved expanding the Lesson 1 pattern across all 43 lessons in Modules 1–5. `src/moduleActionPlans.ts` owns each additional lesson's starting route, optional answers, supplied material and Check-to-answer repair mapping. `src/lessonActions.ts` combines that plan with the existing teaching and guided practice. Lesson 1 retains its original action source and IDs. The app, server and generated Markdown consume the same resulting flow. See [VERIFICATION-MODULE-ACTIONS.md](VERIFICATION-MODULE-ACTIONS.md) for the per-lesson inventory and actual QA.

Every future action flow must retain its authored demonstrations and supported attempts, name a concrete starting artifact or honest missing-access route, expose one editable answer at a time, and return Check feedback to relevant answer fields. Preserve all worksheet and saved question IDs. Mark optional and conditional fields explicitly; do not require invented participant evidence. Blank optional fields do not earn progress, and required fields depend on the learner's declared route. Existing explicit finishes remain valid for time/navigation updates; editing substantive work reopens practice under the current requirements.

Desktop may place task context beside a readable answer column. Phones keep one column, wrapping labels, at least 44 px controls, 16 px editable text and a bounded lesson overview. Timer controls keep a stable footprint while starting or pausing. Verify real clicks, long answers, repair selection, saved resume and offline recovery as well as width. Keep Haru observation, native phone checks and formal mastery separate from implementation evidence.

## Small action and tracking contract — 12 September 2026

This contract supersedes earlier instructions to keep all formative answers ephemeral or to float the timer. Preserve the original learning sequence, curriculum and full Lesson 1 workload. Publication, interface implementation, lesson refinement, learner validation and assessment are separate states.

- Keep cream/green/serif styling, Learn → Do → Check → Your work, keyboard access and direct section navigation. One required action and its answer control occupy the main card. Use an overview for revisiting; do not force a long list to remain visible.
- Every action states what to do, where to do it and what is enough. Give a concrete example before requesting independent work. Keep definitions, material labels, tool controls and stop conditions visible. Deeper reading and videos are optional; do not repeat setup, save directions, timers or worksheet summaries around every action.
- A question must collect a choice or reason before feedback. Feedback explains the reasoning and leads to a bounded repair in the learner's own answer, edited beside that feedback. An honest explanation that no repair was needed is valid; do not force a fabricated mistake.
- Preserve the five legacy step indices used by session logs. Lesson.flow contains stable action IDs, section IDs, legacy step attribution and references to worksheet fields or supplied checks. Lesson 1 is authored in src/lessonOneFlow.ts; other reviewed flows use src/lessonActions.ts with explicit early-module plans or later-module plans derived from the lesson-specific route and repair instructions. Each of the 2,866 worksheet field IDs appears exactly once in its lesson flow. Labels may change; IDs may not.
- The supplied walkthrough is fictional practice, explicitly labelled throughout. Neither it nor a personal walkthrough is participant research. Existing required outputs stay required; do not reduce them by hiding fields or pre-filling answers. External files remain an honest alternate route with an output checklist, reflection and reference, not an upload.
- Automatic time starts with meaningful input after record hydration. It counts only visible, focused lesson use, stops on blur/background/exit or after five minutes without input, and resumes on interaction unless explicitly paused. No wall-clock absence is reconstructed after reload or sleep. Reading beyond the quiet limit, other apps and paper work are added manually. The inline timer cannot cover navigation or the keyboard.
- Lesson work measures completed required activities. Module/course practice finished counts explicit finished markers across published required teaching lessons (currently 221); the diagnostic and three optional lessons are separate. No reading, guide tick, time total, confidence or legacy review request becomes a finished marker automatically. Formal mastery scoring remains unimplemented.
- Finish practice requires worksheet work (or external-work confirmation), all formative checks attempted and explained, and a repair or reason no repair was needed. It is separate from requesting creator feedback. Editing work reopens practice; time and navigation do not. Apply these rules server-side too.
- RecordData keeps version 1 and adds optional bounded learning state (action, completed actions, answers with shown feedback, route, output confirmation, repair and finishedAt), timed segment UUIDs/elapsedMs, and timingRemainderMs. GET/PUT progress, course summaries, backups, review exports and MCP preserve the fields. No new database table is needed. Never reuse an ID for different content or silently merge stale drafts.
- Time checkpoints are local every 15 seconds and folded into the record every minute or on transitions. Absolute segment elapsed time prevents retry duplication; fractional minutes carry across segments. History remains capped at 50 entries and authoritative total minutes may exceed the retained history. Corrections preserve segment identity and cap logged minutes to the corrected total.
- Browser tests must exercise 320/390px, tablet and desktop, long text, keyboard navigation, direct section access, exact-action resume, offline/reconnect, concurrent revisions, multiple tabs and account isolation. Native soft-keyboard and Haru observation are separate evidence; never claim them from desktop emulation.

All modules now use the reader. Do not infer learner understanding, formal assessment or mastery from this implementation. Keep historical verification evidence and add current results to PROGRESS.md and the relevant verification document.
...
| `checks` | `ActiveCheck[]` of `question`, `options[]`, `repair`, `recheck` | "Check the reason" and "Improve your work". The learner answers before feedback; the choice and reveal are saved as formative practice, never scored as mastery. `repair` must send one specific change back into the learner's own artifact, and the lesson should carry a field where that change is recorded |
