...
- Preserve the five legacy step indices used by session logs. Lesson.flow contains stable action IDs, section IDs, legacy step attribution and references to worksheet fields or supplied checks. It is authored once in src/lessonOneFlow.ts and used by the reader, generated Markdown and MCP. Each of the 36 Lesson 1 field IDs appears exactly once in the action flow. Labels may change; IDs may not.
...
Remaining lessons inherit shared time, Check persistence and explicit completion controls only. Review each lesson before adding its flow; do not batch-mark them redesigned. Keep historical verification evidence and add current results to PROGRESS.md and VERIFICATION-ACTIONS.md.
