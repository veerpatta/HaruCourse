...
## week1-day1-v1 rewritten and extended — 7 September 2026

The creator asked for lesson one to be improved further: plainer English, and one more thing to practise on. Two changes followed.

**The English.** The guided material added by the earlier rework was written plainly; the lesson body underneath it was not, and it is the first thing a learner reads. It opened with "Product design connects a user goal to a usable, viable service" and used UX and UI without expanding either. Every visible sentence of `week1-day1-v1` was rewritten in `src/lessons.ts`: the reason to care, the five teaching bullets, the example, the four explanation paragraphs, what to bring, the outputs, the repairs, all five steps' instructions, both question-and-answer pairs, the rubric and the portfolio note. "Viable" is gone from both places it appeared. UX and UI are expanded on first use. Observed, inferred and unknown are now introduced in Learn, where the earlier version left a learner to meet the lesson's central distinction for the first time in step 3, at the moment she was asked to use it. The guided material in `src/apprenticeship.ts` had the same pass over its mission, hints, standard, handoff and field help. The two improvement fields no longer share the label "What would show whether it helped?", which had been printing twice in the downloadable copy, and the business-goal field now explains what a business goal is instead of assuming it.

**The evidence sorter.** Step 3 turns on one distinction and used to practise it once. It now carries six supplied, explicitly made-up note lines; each is labelled observed, inferred or unknown, and every one of the three labels has its own explanation for that line, so a plausible wrong answer teaches. It replaces the single supported case, which was folded in as its first line, and the step-level example, which repeated what the demonstration already showed. A closing observation names what the two hardest lines had in common rather than reporting a tally or generalising about other learners. Nothing about it is saved, marked or scored: answers live in component state, so there is no stored state, no D1 write and no server work per interaction. Step 4 also gained a demonstration, of a "how would I know?" line that could not fail being thrown away and replaced with one that could.

`GuideStep.sorter` is a new optional member any lesson may carry. The content checker gained rules for it: the supplied material must say it is made up, three to eight lines, unique line ids, an answer drawn from the offered labels, an explanation for every label on every line, and the same rule against feedback that repeats the verdict the interface already shows. That last rule was hoisted out of `oneCorrect` so both shapes share it. A refined lesson must now carry a supported case or a sorter before it asks for unaided work, and the diagnostic may carry neither.

### Checks run

- `docs:generate`, `test:content`, `test:worksheet`, both typechecks and `build` pass. Only `WEEK-01.md` changed among generated documents. Main chunk 2,773.52 kB uncompressed, 804.18 kB gzip, 6.13 kB above the previous release.
- All thirteen backend groups pass against the local Worker. No schema, storage or API change.
- The lesson was read back at runtime, not merely built: the sorter reaches the lesson object on step 3 with six lines and complete feedback, three demonstrations sit on steps 1, 3 and 4, no supported case remains, the worksheet still declares 36 fields, no visible string exceeds the 240-character limit, no field example lost the literal words "made up", and no worksheet text contains a pipe character.
- Browser, local Worker on 8788, test account only. The rewritten Learn text renders. Step 3 shows the sorter with six line-by-line questions and eighteen options and no feedback until a choice is made; a wrong choice returns the explanation for that label on that line; changing to the defensible label swaps the explanation and marks it right; the closing observation appears only once all six are answered. The practice record fetched before and after answering all six is byte-identical. A worksheet answer typed into step 3 reported "Saved online", was present in the server record, and survived a reload with the reveal still showing it; the sorter answers reset on reload, as intended. Step 4's demonstration renders, and steps 1, 3 and 4 head their worksheet "Try it yourself" while 2 and 5 head it "Fill in". At 320 px and 390 px nothing in the sorter or the worksheet exceeds the viewport, option rows are 44 px, and focusing a radio draws the 3 px green outline around the whole row.
- `scripts/test-worksheet.mjs` pinned two field labels and one section title as exact Markdown. Field ids are record keys and stay frozen; labels are display text and must be free to get plainer. The test now looks those labels up by id, so it still proves label-then-value rendering, a multi-line answer surviving and a choice value round-tripping, without blocking a rewrite.

### Not verified in this pass

- Haru has not used it. No learner validation is claimed.
- No screenshots. The browser pane would not paint while the application window was hidden; every browser check above is from the accessibility tree, page text and measured geometry instead.
