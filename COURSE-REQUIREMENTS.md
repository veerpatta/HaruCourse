...
| 12. Prototyping | Levels 1–3, advanced Level 5 | Intro only: bounded paper/clickable prototype | Interactive components, variables, conditions, advanced workflows and evaluated presentations |
| 13. Accessibility | Integrated plus dedicated Level 3 work | Intro only: labels, focus intent, evidence limits | WCAG teaching, contrast, keyboard/screen-reader testing, targets, forms, components and inclusive design exercises |
| 14. Product analytics | Level 5 | Planned | Funnels, activation, conversion, retention, engagement, dashboards, quantitative interpretation and A/B limitations |
| 15. AI + Product Design | Level 5 with responsible-use habits earlier | Planned teaching; working MCP support | Research/ideation/wireframing/prototyping assistance, evaluation, conversational UX, agents, human control, responsible patterns and when not to use AI |
| 16. Product delivery | Level 4 and all major projects | Planned | Feasibility, PM/engineering collaboration, APIs, tickets, acceptance criteria, Agile/Scrum, QA, versioning and release exercises |
| 17. Advanced practice | Level 5 and post-readiness extension | Planned | Strategy, service/systems thinking, enterprise/B2B/consumer/commerce/AI breadth, leadership, stakeholder management, advocacy and measured impact |

## Required supporting programs

| Requirement | Current state | What must be authored or implemented |
|---|---|---|
| Complete self-paced module roadmap | High-level 620-hour blueprint; m00 baseline plus m01–m05 published, 43 authored lessons | The full module map covers all 17 areas; publish the remaining 15 modules in prerequisite order, starting at m06 |
...
| Current resources | 59 verified public readings, ten of them added for m05 and retrieved on 6 September 2026. Of the eight recorded subtopic gaps, six are sourced and two are partial: market positioning lacks a verified free source, and hiring has two dated vacancy captures that are explicitly not a study. Authenticated tool workflows remain open but nothing depends on them, since R49 gives an account-free vector path | Find a verified market-positioning source; run the multi-employer hiring pass by hand at m20; keep per-row purpose, level, cost status, link, checked date and fallback current at each release |
...
The direction is aligned, but the deliverable is substantially incomplete. Forty-three published lessons across five modules cover Level 1 and the research half of Level 2, and represent an introduction with one area worked properly: fifteen modules remain unauthored, including information architecture, flows and wireframes in the same level. Written criteria and remediation are not an assessment system, and no learner work has been assessed. Future milestones should continue rebalancing toward content and assessment rather than platform.
...
1. Follow the mapped prerequisite sequence in COURSE-BLUEPRINT.md. m03, m04 and m05 are authored and published; m06 is next. Information architecture needs a verified tree-testing source before its lesson can be written, since the catalog's structural-IA row explicitly does not cover card-sort or tree-test methodology.
