...
The creator explicitly authorized completing the beginner-comprehension work for Modules 11–20, committing and pushing the complete change set to `main`, deploying it to the live Cloudflare site and running save, backend and progress smoke tests with the isolated `test` identity. This authorization supersedes the earlier planning-only and wait-before-expansion limits for this implementation. Preserve all published lesson and worksheet IDs, record version 1, prior learner records, the complete workload, independent diagnostic and the distinction between recorded work, explicit practice completion, creator review and mastery. Use [the all-course beginner verification](docs/VERIFICATION-BEGINNER-ALL-LESSONS.md) for release evidence and keep learner observation and formal assessment open until they actually occur.

## Latest requested refinement — beginner understanding, 13 September 2026

Reported feedback: the course feels detailed but confusing for a beginner, with unfamiliar terms and insufficiently intuitive what/where/how guidance. The creator requests a researched, creative plan covering all lessons, including optional external-AI learning activities and clear timer/progress language. This milestone changes Markdown only; it does not implement, publish or validate the proposed experience.

The [beginner experience plan](docs/BEGINNER-EXPERIENCE-PLAN.md) defines the proposed standard and rollout. [Research comparisons](docs/BEGINNER-RESEARCH.md), [AI prompt/activity examples](docs/AI-LEARNING-ACTIVITIES.md), and [the 224-lesson map](docs/BEGINNER-LESSON-PLAN.md) make the proposal reviewable. The original vision and existing competencies/workload stay intact. Teach in clear adult language using familiar situations, visible explanations, demonstrated reasoning, supported attempts and independent application. Keep optional depth without hiding what a beginner needs. Full bilingual delivery is not assumed; support a preferred explanation language in optional AI practice while retaining actual tool labels.

Each AI activity needs lesson-specific context, a complete copyable prompt, one question at a time, help when stuck, a stop, an exact return task and a complete non-AI alternative. AI answers are not automatically accurate/current and never become participant evidence or mastery. Preserve the independent baseline. Keep progress based on required work and explicit practice finish; keep course activity time automatic, away time manual and review separate. All rows in the new lesson plan start Planned. Existing action coverage and technical QA do not establish learner comprehension.
