...
const labelOf = (id) => lesson.apprenticeship.worksheet.flatMap(w => w.fields).find(f => f.id === id).label;
assert.ok(md.includes(`**${labelOf("define-ux")}**\nQA line one\nline two`));
assert.ok(md.includes(`**${labelOf("entry-1-label")}**\nobserved`));
